...
<body>
	...
	<body>
			...
			<?php
				...
				$sql = "SELECT A.*,B.nome from servicos A  LEFT JOIN usuarios B ON A.idusuario=B.idusuario where (razao like :nome1 or descricao like :nome2) and A.idgrupo=:idgrupo and status=:status_filtro  order by descricao limit :inicio,30";
					else 
				if ($idempresa!=0)
					$sql = "SELECT A.*,B.nome from servicos A  LEFT JOIN usuarios B ON A.idusuario=B.idusuario where (razao like :nome1 or descricao like :nome2) and A.idgrupo=:idgrupo and A.idempresa=:idempresa and status=:status_filtro order by descricao limit :inicio,30";	
				else $sql = "SELECT A.*,B.nome from servicos A  LEFT JOIN usuarios B ON A.idusuario=B.idusuario where (razao like :nome1 or descricao like :nome2) and A.idgrupo=:idgrupo and A.idusuario=:idusuario and status=:status_filtro order by descricao limit :inicio,30";	
					$stmt = $pdo->prepare($sql);
				$stmt->bindValue(':nome1', "%" . $nome . "%");
				$stmt->bindValue(':nome2', "%" . $nome . "%");
                $stmt->bindValue(':status_filtro', $status_filtro);
				if($iduser!=$idgrupo)	{
				...
				$stmt->bindParam(':inicio', $inicio, PDO::PARAM_INT);
					...
					if ($idempresa!=0)
					$sql = "SELECT A.*,B.nome as NomeUsuario from servicos A  LEFT JOIN usuarios B ON A.idusuario=B.idusuario where (razao like :nome1 or descricao like :nome2) and A.idgrupo=:idgrupo and A.idusuario=:idusuario and A.idempresa=:idempresa and status=:status_filtro  order by descricao limit :inicio,30";	
				else $sql = "SELECT A.*,B.nome from servicos A  LEFT JOIN usuarios B ON A.idusuario=B.idusuario where (razao like :nome1 or descricao like :nome2) and A.idgrupo=:idgrupo and A.idusuario=:idusuario  and status=:status_filtro order by descricao limit :inicio,30";		
